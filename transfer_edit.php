...
if ($action == 'delete_transfer') {
    ...
    // == prepare transfer card
        ...
        $transfer->date1 =  $_POST['date1year'].str_pad($_POST['date1month'], 2, "0", STR_PAD_LEFT).str_pad($_POST['date1day'], 2, "0", STR_PAD_LEFT).'000001';
        
        if (!empty($_POST['date2year'])) // == d/m/Y -> %Y%m%d%H%M%S
        $transfer->date2 =  $_POST['date2year'].str_pad($_POST['date2month'], 2, "0", STR_PAD_LEFT).str_pad($_POST['date2day'], 2, "0", STR_PAD_LEFT).'000001';
