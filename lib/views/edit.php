    ...
    <form action="<?= $_SERVER["PHP_SELF"] ?>" method="POST" name="transfer_card_form" id="transfer_card_form">
        ...
        <table class="border" style="width:100%;">
            ...
            <tr>
                ...
                <td><?= $form->select_date(!empty($transfer->date2) ? $db->jdate($transfer->date2): '','date2',0,0,1,"transfer_card_form",1,1) ?></td>
            </tr>
            <tr>
...
<script>
    function js_delete_transfer(){
        if (confirm("<?= str_replace('"','',html_entity_decode($langs->trans('stocktransfersDelSure','',0),ENT_QUOTES)) ?>")){
    ...
    function js_set_as_sent(){
        if ($('#transfer_card_form input[name=date1]').val()==''){
            ...
            alert("<?= str_replace('"','\"',  html_entity_decode($langs->trans('stocktransfersErrorMsg01'),ENT_QUOTES)) ?>");
        }else{
    ...
    function js_set_as_received(){
        if ($('#transfer_card_form input[name=date2]').val()==''){
            ...
            alert("<?= str_replace('"','\"',  html_entity_decode($langs->trans('stocktransfersErrorMsg02'),ENT_QUOTES)) ?>");
        }else{
